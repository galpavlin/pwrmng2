...
void tx_byte(uint8_t byte)
{
	if(endoftx == 0)
	{
		tx_buffer_comm[length] = byte; //fill buffer
		length++;
	}
	else
	{
		tx_buffer_comm[length] = byte; //add end of frame byte
		length++;
		USB_write(tx_buffer_comm,length); //transmit whole buffer
